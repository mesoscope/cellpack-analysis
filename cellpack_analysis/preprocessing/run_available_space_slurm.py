...
def _wait_for_jobs(job_ids: list[str], poll_interval: int = POLL_INTERVAL_SECONDS) -> None:
...
def _run_single_cell(cell: dict[str, Any], manifest: dict[str, Any]) -> str:
    ...
    calculate_grid_distances(
        ...
        calc_scaled_z_distances=manifest.get("calc_scaled_z_distances", True),
...
def _run_orchestrator(
    ...
    staging_dir = base_datadir / f"structure_data/{structure_id}/slurm_staging"
    manifest_dir = staging_dir / "manifests"
    script_dir = staging_dir / "scripts"
    result_dir = staging_dir / "results"
    log_dir = base_datadir / f"structure_data/{structure_id}/slurm_logs"
    for d in [manifest_dir, script_dir, result_dir, log_dir]:
        d.mkdir(parents=True, exist_ok=True)

    mesh_data = _discover_mesh_data(
        structure_id, use_mean_shape, use_struct_mesh, mesh_folder, use_all_cells
    )
    if not mesh_data:
...
""",
    ...
    orch.add_argument(
        ...
        action="store_true",
        default=True,
        help="Return after submission without waiting (default: True).",
    )
