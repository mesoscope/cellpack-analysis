...
for gene, df_gene in meta_df.groupby("structure_name"):
    ...
    volume_per_unit[valid_counts] = df_gene["STR_shape_volume"].to_numpy()[valid_counts] / counts[valid_counts]
    ...
    sphericity[valid_sphericity] = (mem_surface_area[valid_sphericity] ** 1.5) / mem_volume[valid_sphericity]
    ...
    if gene in structures_of_interest:
        ...
        if np.any(valid_data):
            ...
            print(f"  Volume: {np.nanmean(volume_per_unit):0.2f} ± {np.nanstd(volume_per_unit):.2f}")
