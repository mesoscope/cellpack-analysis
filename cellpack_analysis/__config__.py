...
def make_config_from_dotlist(module: ModuleType, args: list[str]) -> DictConfig:
    structure_config = generate_config(module.StructureConfig, "structure", args)
    series_config = generate_config(module.SeriesConfig, "series", args)
    recipe_config = generate_config(module.RecipeConfig, "recipe", args)
...
def make_config_from_yaml(module: ModuleType, args: list[str]) -> DictConfig:
    ...
    config_dataclass = make_dataclass(
        ...
        [
            ...
            ("structure", module.StructureConfig, MISSING),
            ("series", module.SeriesConfig, MISSING),
            ("recipe", module.RecipeConfig, MISSING),
        ],
    )
...
def make_config_from_file(module: ModuleType, file: str) -> DictConfig:
    ...
    structure_config = load_config(module.StructureConfig, contents.structure)
    series_config = load_config(module.SeriesConfig, contents.series)
    recipe_config = load_config(module.RecipeConfig, contents.recipe)
