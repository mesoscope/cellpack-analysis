...
def get_mean_shape_as_image(control):

    device = io.LocalStagingIO(control)
    space = shapespace.ShapeSpaceBasic(control)
    variables = control.get_variables_values_for_aggregation()
    df_ms = space.get_aggregated_df(variables).drop(columns=["structure"]).drop_duplicates().reset_index(drop=True)
    df_ms = df_ms.loc[df_ms.mpId==control.get_center_map_point_index()].head(1)

    device.row = df_ms.iloc[0]
