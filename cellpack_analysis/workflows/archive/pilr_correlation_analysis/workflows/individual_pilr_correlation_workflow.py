...
DEFAULT_CONFIG_PATH = (
    project_root
    / "cellpack_analysis/analysis/pilr_correlation_analysis/configs/peroxisome_individual_pilr_correlation.json"
)
...
def calculate_and_save_correlations(config_path: Path = DEFAULT_CONFIG_PATH):
    ...
    for structure_name, structure_info in config["data"].items():
        ...
        for rule, rule_info in structure_info["rules"].items():
            ...
            for i, cell_id in enumerate(cell_id_list):
                ...
                structure_rule_cell_id_lookup.append((structure_name, rule, cell_id))
    pilr_array = np.array(pilr_list)

    # Create pairs of structures and rules to compare
    index_pairs = list(itertools.combinations_with_replacement(range(len(pilr_array)), 2))
    ...
    structure_rule_cell_id_pairs = [
        [
            structure_rule_cell_id_lookup[i][0],
            structure_rule_cell_id_lookup[j][0],
            structure_rule_cell_id_lookup[i][1],
            structure_rule_cell_id_lookup[j][1],
            structure_rule_cell_id_lookup[i][2],
            structure_rule_cell_id_lookup[j][2],
        ]
